...
[Note] : This program is still under test. I'm trying to find testcases where it won't work. If found please don't hesitate to share it with me. Thank you.

	Problem Statement: Given an Array of sorted Integers find a Triplet 
			where the sum of the triplet is equal to the target sum provided
        
	Time Complexity**: O(n) {O(n) for hasing the dictionary and O(n) then
           	for traversing once through the dictionary
